...
alter table public.predictions
  add constraint predictions_exact_value_consistency
  check (
    (side = 'exact' and exact_value is not null)
    or (side <> 'exact' and exact_value is null)
  );
...
begin
  ...
  elsif p_exact_value is not null then
...
revoke all on function public.place_prediction(uuid, public.prediction_side, int, int)
from public, anon, authenticated;

grant execute on function public.place_prediction(uuid, public.prediction_side, int, int)
to authenticated;
